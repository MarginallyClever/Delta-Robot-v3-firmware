...
void setup_robot() {
  ...
  for(i=0;i<NUM_AXIES;++i) {
    Arm &a=robot.arms[i];
    // shoulder
    a.shoulder=Vector3(cos(TWOPI*(float)i/(float)NUM_AXIES)*CENTER_TO_SHOULDER,  
                       sin(TWOPI*(float)i/(float)NUM_AXIES)*CENTER_TO_SHOULDER,
                       CENTER_TO_FLOOR);
    ...
    // elbow
    a.elbow.pos=Vector3(cos(TWOPI*(float)i/(float)NUM_AXIES)*(CENTER_TO_SHOULDER+SHOULDER_TO_ELBOW),
                        sin(TWOPI*(float)i/(float)NUM_AXIES)*(CENTER_TO_SHOULDER+SHOULDER_TO_ELBOW),
                        CENTER_TO_FLOOR);
    // Find wrist position.
    a.elbow.relative=a.elbow.pos;
    a.wrist.relative=Vector3(cos(TWOPI*(float)i/(float)NUM_AXIES)*(EFFECTOR_TO_WRIST),
                             sin(TWOPI*(float)i/(float)NUM_AXIES)*(EFFECTOR_TO_WRIST),
                             0);
